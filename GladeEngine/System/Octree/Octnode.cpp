...
Octnode::Octnode(Octree* t, Vector center, gFloat width, gFloat height, gFloat depth, uint64_t loc) :  tree(t), numElements(0), numElementsBranch(0), locationCode(loc),
																										boundingBox(center, width*gFloat(0.5f), height*gFloat(0.5f), depth*gFloat(0.5f))
{
...
void Octnode::Update()
{
	...
	// If immediate branch (self and children) has a small amount of elements, merge together and destroy children
	if(children > 0)
	{
		assert(numElementsBranch >= numElements);
	...
	GraphicsLocator::GetDebugGraphics()->PushBox(boundingBox.center, boundingBox.GetExtents(), DebugDraw::Color(1,1,1,1));
}
...
{
	gFloat w = boundingBox.Halfwidth();
	gFloat h = boundingBox.Halfheight();
	gFloat d = boundingBox.Halfdepth();

	if(i & 1<<0 && !(children & 1<<0))
		tree->CreateNode(new Octnode(tree, Vector(boundingBox.center.x - w*gFloat(0.5f), boundingBox.center.y - h*gFloat(0.5f), boundingBox.center.z - d*gFloat(0.5f)),
			w, h, d, (locationCode<<3)|0));
	if(i & 1<<1 && !(children & 1<<1))
		tree->CreateNode(new Octnode(tree, Vector(boundingBox.center.x + w*gFloat(0.5f), boundingBox.center.y - h*gFloat(0.5f), boundingBox.center.z - d*gFloat(0.5f)),
				w, h, d, (locationCode<<3)|1));
	if(i & 1<<2 && !(children & 1<<2))
		tree->CreateNode(new Octnode(tree, Vector(boundingBox.center.x - w*gFloat(0.5f), boundingBox.center.y - h*gFloat(0.5f), boundingBox.center.z + d*gFloat(0.5f)),
				w, h, d, (locationCode<<3)|2));
	if(i & 1<<3 && !(children & 1<<3))
		tree->CreateNode(new Octnode(tree, Vector(boundingBox.center.x + w*gFloat(0.5f), boundingBox.center.y - h*gFloat(0.5f), boundingBox.center.z + d*gFloat(0.5f)),
				w, h, d, (locationCode<<3)|3));
	if(i & 1<<4 && !(children & 1<<4))
		tree->CreateNode(new Octnode(tree, Vector(boundingBox.center.x - w*gFloat(0.5f), boundingBox.center.y + h*gFloat(0.5f), boundingBox.center.z - d*gFloat(0.5f)),
				w, h, d, (locationCode<<3)|4));
	if(i & 1<<5 && !(children & 1<<5))
		tree->CreateNode(new Octnode(tree, Vector(boundingBox.center.x + w*gFloat(0.5f), boundingBox.center.y + h*gFloat(0.5f), boundingBox.center.z - d*gFloat(0.5f)),
				w, h, d, (locationCode<<3)|5));
	if(i & 1<<6 && !(children & 1<<6))
		tree->CreateNode(new Octnode(tree, Vector(boundingBox.center.x - w*gFloat(0.5f), boundingBox.center.y + h*gFloat(0.5f), boundingBox.center.z + d*gFloat(0.5f)),
				w, h, d, (locationCode<<3)|6));
	if(i & 1<<7 && !(children & 1<<7))
		tree->CreateNode(new Octnode(tree, Vector(boundingBox.center.x + w*gFloat(0.5f), boundingBox.center.y + h*gFloat(0.5f), boundingBox.center.z + d*gFloat(0.5f)),
				w, h, d, (locationCode<<3)|7));

	children |= i;
}

void Octnode::Merge() // more like ASSIMILATE
{
...
{
	AABB test(boundingBox.center, (boundingBox.Halfwidth() * tree->looseness) - rad, 
									(boundingBox.Halfheight() * tree->looseness) - rad,
									(boundingBox.Halfdepth() * tree->looseness) - rad);

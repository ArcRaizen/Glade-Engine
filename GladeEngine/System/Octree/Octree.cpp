...
Octree::Octree(OctreeType t, bool resize, Vector center, gFloat width, gFloat height, gFloat depth, int maxE, int minE, gFloat loose) : type(t), resizeable(resize), maxElementsPerPartition(maxE >= 1 ? maxE : 1), 
																																		...
																																		looseness(loose), boundingBox(center, width*gFloat(0.5f),height*gFloat(0.5f),depth*gFloat(0.5f)), 
																																		beganUpdate(false)
{
...
void Octree::Update()
{
	...
	beganUpdate = true;
	// std::map is already sorted by operator< order.
...
void Octree::Grow(Object* o)
{
	...
	do
	{
		...
		AABB rootBox = nodes[1]->GetBox();
		Vector diff = o->GetPosition() - rootBox.center;
		diff.x = diff.x == 0 ? 0 : (diff.x / Abs(diff.x)) * rootBox.Halfwidth();
		diff.y = diff.y == 0 ? 0 : (diff.y / Abs(diff.y)) * rootBox.Halfheight();
		diff.z = diff.z == 0 ? 0 : (diff.z / Abs(diff.z)) * rootBox.Halfdepth();;

		// Create new Root Node with the nearest corner of current root node as center
		newRoot = new Octnode(this, rootBox.center+diff, rootBox.Halfwidth()*4, rootBox.Halfheight()*4, rootBox.Halfdepth()*4, 1);
...
{
	assert(!LookUpNode(locCode)->HasChildren()); // TODO - UTILS/ASSERT
